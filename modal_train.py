"""Run full training and evaluation on Modal - matching paper targets."""

import modal

app = modal.App("rlci-training-v5")
...
def run_training():
    ...
    print("=" * 70)
    print("FULL TRAINING AND EVALUATION (β=20)")
    ...
    print(
        f"  Actions: full={rl_metrics['action_distribution'][0]} ({rl_metrics['action_distribution'][0]/total*100:.1f}%), "
    ...
    tp_improvement = (
        (rl_metrics["throughput"] - sb_metrics["throughput"])
        / sb_metrics["throughput"]
        * 100
    )
    print(f"\n{'='*60}")
    ...
    print("\n[3/5] Beta sensitivity study β ∈ {1, 3, 5, 10, 15, 20, 30, 40}...")
    for beta in [1.0, 3.0, 5.0, 10.0, 15.0, 20.0, 30.0, 40.0]:
        agent_b, _ = train(
            num_episodes=2000,
            commits_per_episode=100,
            beta=beta,
        ...
        tp_b = (
            (m["throughput"] - sb_b["throughput"]) / sb_b["throughput"] * 100
            if sb_b["throughput"] > 0
            else 0
        )
        print(
            f"  β={beta:>4.0f}: TP_imp={tp_b:>6.1f}% | TTS={m['test_time_savings']*100:>5.1f}% | "
            f"DMR={m['defect_miss_rate']*100:>5.1f}% | "
            f"full={m['action_distribution'][0]/total_b*100:.0f}% "
            f"partial={m['action_distribution'][1]/total_b*100:.0f}% "
            f"skip={m['action_distribution'][2]/total_b*100:.0f}%"
        )

    # 4. Adversarial robustness
    print("\n[4/5] Adversarial robustness evaluation...")
    adv_metrics = adversarial_evaluation(
        agent, num_episodes=500, seed=100, beta=BETA
    )
    ...
    for run in range(5):
        ...
        print(
            f"  Run {run+1}: TP_imp={tp_i:.1f}%, TTS={m['test_time_savings']*100:.1f}%, DMR={m['defect_miss_rate']*100:.1f}%"
        )

    print(f"\n  Mean ± Std:")
    print(
        f"    TP improvement: {np.mean(tp_runs):.1f}% ± {np.std(tp_runs):.1f}%"
    )
    print(
        f"    Test time savings: {np.mean(tts_runs):.1f}% ± {np.std(tts_runs):.1f}%"
    )
    print(
        f"    Defect miss rate: {np.mean(dmr_runs):.1f}% ± {np.std(dmr_runs):.1f}%"
    )
